...
class apiJson:
    ...
    def savePDF(self, newpdfText, getCSList, titleList):
        print("*" * 10, "读取结果存入txt", "*" * 10)
        saveTxt("识别PDF_%s" % self.pdfName, newpdfText)
        print("*" * 10, "爬取结果存入excel", "*" * 10)
        saveExcel("提取参数_%s" % self.pdfName, getCSList, type=1)
        saveExcel("提取结构_%s" % self.pdfName, titleList, type=2)
